...
while XD.Functions.GetPlayerData().job == nil do
    Citizen.Wait(10)
end
    ...
    while true do
        ...
        if isLoggedIn then
            if PlayerJob.name ~= nil then
                ...
                for k, v in pairs(XD.Shared.Jobs[PlayerJob.name]["boss"]) do
...
AddEventHandler('cash_bossmenu:client:refreshSociety', function(job, data)
    if XD and XD.Functions.GetPlayerData().job.name == job then
        SendNUIMessage({
            open = true,
...
RegisterNUICallback('openStash', function(data)
    ...
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "boss_" .. XD.Functions.GetPlayerData().job.name, {
        maxweight = 4000000,
        slots = 500,
    })

    TriggerEvent("inventory:client:SetCurrentStash", "boss_" .. XD.Functions.GetPlayerData().job.name)
end)
