...
AddEventHandler("cash_bossmenu:server:openMenu", function()
    ...
    local employees = {}
    print("BOSS MENU - Copyrigth FivemScipt - CoderC-SLo enter and find out in our discord:  https://discord.gg/RMybeEU ")
    if job.isboss == true then
        if not Accounts[job.name] then
            Accounts[job.name] = 0
        end

        XD.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `job` LIKE '%".. job.name .."%'", function(players)
            ...
            TriggerClientEvent('cash_bossmenu:client:openMenu', src, employees, XD.Shared.Jobs[job.name])
...
AddEventHandler('cash_bossmenu:server:fireEmployee', function(data)
    ...
    if xEmployee then
        if xEmployee.Functions.SetJob("unemployed", '0') then
            ...
            Wait(500)
            local employees = {}
            XD.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `job` LIKE '%".. xPlayer.PlayerData.job.name .."%'", function(players)
                if players[1] ~= nil then
                    for key, value in pairs(players) do
                        local isOnline = XD.Functions.GetPlayerByCitizenId(value.citizenid)
        ...
        XD.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `citizenid` = '" .. data.source .. "' LIMIT 1", function(player)
            if player[1] ~= nil then
                ...
                XD.Functions.ExecuteSql(false, "UPDATE `players` SET `job` = '"..json.encode(job).."' WHERE `citizenid` = '".. data.source .."'")
                TriggerClientEvent('XD:Notify', src, "Fired successfully!", "success")
...
AddEventHandler('cash_bossmenu:server:giveJob', function(data)
    ...
    if xPlayer.PlayerData.job.isboss == true then
        if xTarget and xTarget.Functions.SetJob(xPlayer.PlayerData.job.name) then
            TriggerClientEvent('XD:Notify', src, "You recruit " .. (xTarget.PlayerData.charinfo.firstname .. ' ' .. xTarget.PlayerData.charinfo.lastname) .. " to " .. xPlayer.PlayerData.job.label .. ".", "success")
            TriggerClientEvent('XD:Notify', xTarget.PlayerData.source , "You've been recruited to " .. xPlayer.PlayerData.job.label .. ".", "success")
...
AddEventHandler('cash_bossmenu:server:updateGrade', function(data)
    ...
    if xEmployee then
        ...
        XD.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `citizenid` = '" .. data.source .. "' LIMIT 1", function(player)
            if player[1] ~= nil then
                xEmployee = player[1]
                local job = XD.Shared.Jobs[xPlayer.PlayerData.job.name]
                local employeejob = json.decode(xEmployee.job)
                employeejob.grade = job.grades[data.grade]
                XD.Functions.ExecuteSql(false, "UPDATE `players` SET `job` = '"..json.encode(employeejob).."' WHERE `citizenid` = '".. data.source .."'")
                TriggerClientEvent('XD:Notify', src, "Promoted successfully!", "success")
